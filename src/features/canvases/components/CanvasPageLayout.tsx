...
export function CanvasPageLayout<T>({
  ...
  // Trigger load when project ID is set
  useEffect(() => {
    if (currentProjectId) {
      projectMgmt.load()
    }
  ...
  const handleCopySlides = async () => {
    ...
    try {
      ...
      toast.success('Markdown скопирован в буфер обмена! Вставьте его в презентацию.')
    } catch (err) {
      toast.error('Не удалось скопировать markdown')
      console.error(err)
    }
  }
