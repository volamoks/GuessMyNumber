 ...
 * Поддерживает форматы: #RRGGBB, #RGB, RRGGBB
 */
export function hexToColor(hex: string | undefined | null): string {
  if (!hex) return '000000'

  // Удаляем # и пробелы
  let color = hex.replace(/[^0-9a-fA-F]/g, '')

  // Если короткий формат (RGB), превращаем в RRGGBB
  if (color.length === 3) {
    color = color.split('').map(c => c + c).join('')
  }

  // Если длина не 6, возвращаем черный (fallback)
  if (color.length !== 6) {
    console.warn(`Invalid color format: ${hex}, falling back to black`)
    return '000000'
  }
