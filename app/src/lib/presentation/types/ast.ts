...
// Специальные блоки для Product Tools
export interface RoadmapItemNode extends ASTNode {
  type: 'roadmap_item'
  title: string
  quarter: string
...
export interface CanvasBlockNode extends ASTNode {
  type: 'canvas_block'
  blockType: 'key_partners' | 'key_activities' | 'value_propositions' | 'customer_relationships' | 'customer_segments' | 'key_resources' | 'channels' | 'cost_structure' | 'revenue_streams'
  title: string
  items: string[]
}

export interface GanttTaskNode extends ASTNode {
  type: 'gantt_task'
  id: string
  name: string
  start: string
